...
async function Draw(){
  ...
  oX=0, oY=0, oZ=50
  Rl=t/4, Pt=-t/2, Yw=S(t/3)*10

  shps = [
    subDividedTetrahedron(3,12,(osc=Math.max(0,Math.min(1,(.3-C(t+.25))*10)))*Math.min(2,Math.max(-1,(S(t*6))*4))),
    subDividedCube(3,10,osc*Math.min(3.5,Math.max(-1,(S(t*6))*6))),
    subDividedOctahedron(2,10,osc*Math.min(2,Math.max(-1,(S(t*6))*5))),
    subDividedIcosahedron(2,8,osc*Math.min(8,Math.max(-4,(S(t*6))*16))),
    subDividedDodecahedron(2,10,osc*Math.min(8,Math.max(-4,(S(t*6))*16))),
  ]
  ...
  sp = 24
  shps.map((shp,idx)=>{
    tx = S(p=Math.PI*2/shps.length*idx+t/6)*sp
    ty = C(p)*sp
    tz = 0
    shp.map(v=>{
      x.beginPath()
